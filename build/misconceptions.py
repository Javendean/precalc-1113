...
MISCONCEPTIONS = [
    ...
    # ---- added while authoring the trigonometry / systems / conics items ----
    dict(id="TRIG_CONVERSION_INVERTED", family="RULE_OVERGENERALIZATION",
         name="Uses the degree-radian conversion upside down",
         signature="135 deg -> 4pi/3 instead of 3pi/4",
         kcs=["angles_radians"],
         root="Remembers that 180 and pi are both involved, but not which belongs on top.",
         fix="Anchor on one fact: 180 degrees IS pi radians. Write it as a fraction equal to 1 "
             "and orient it so the unit you are leaving cancels.",
         probe="Convert 90 degrees to radians."),
    dict(id="TRIG_PYTHAG_REARRANGE", family="SIGN_MANAGEMENT",
         name="Rearranges the Pythagorean identity with the wrong sign",
         signature="sin^2 = 1 + cos^2 instead of 1 - cos^2",
         kcs=["trig_identities_basic", "trig_simplify", "trig_equations"],
         root="Moves a term across the equals sign without changing its sign.",
         fix="Check it at one angle: at theta=0, sine is 0 and cosine is 1. Only the minus "
             "version survives that test. Make her run the test rather than trust the memory.",
         probe="Rewrite sin^2(theta) in terms of cosine."),
    dict(id="FORMULA_WRONG_SLOT", family="RULE_OVERGENERALIZATION",
         name="Right formula, wrong slot",
         signature="substitutes diameter for radius, r^2 for r, or inverts a ratio",
         kcs=["angles_radians", "law_sines_cosines", "conics", "vectors", "polar",
              "trig_graphs", "systems_linear", "sequences_series"],
         root="Recalls the formula as a shape and fills it in without checking what each "
              "letter stands for.",
         fix="Have her write out what every letter means BEFORE substituting anything. Most of "
             "these vanish the moment the quantities are named out loud.",
         probe="In s = r theta, which quantity is which?"),
    dict(id="CONIC_AXIS_SWAP", family="NOTATION_LITERALISM",
         name="Attaches the conic's axis to the wrong variable",
         signature="reads the larger denominator as the horizontal axis regardless of position",
         kcs=["conics"],
         root="Reads the two numbers without tracking which variable each sits under.",
         fix="The bigger denominator sits under the variable the long axis runs along. Point at "
             "the variable, not the number.",
         probe="In x^2/9 + y^2/25 = 1, which way is the major axis?"),
    dict(id="SEQ_ARITH_GEOM_MIXUP", family="RULE_OVERGENERALIZATION",
         name="Uses the arithmetic rule on a geometric sequence, or the reverse",
         signature="adds the common ratio, or multiplies by the common difference",
         kcs=["sequences_series"],
         root="Both patterns are stored as 'the next-term rule' without the add-versus-multiply "
              "distinction attached.",
         fix="Ask one question of every sequence before anything else: to get the next term do I "
             "ADD the same number or MULTIPLY by the same number? Write the answer down first.",
         probe="Is 2, 6, 18, 54 arithmetic or geometric?"),
    dict(id="SYS_PARTIAL_SOLUTION", family="INCOMPLETE_SOLUTION_SET",
         name="Reports only part of the solution",
         signature="gives x but not y, or one intersection point when there are two",
         kcs=["systems_linear", "systems_nonlinear"],
         root="Treats the solve as finished when the first unknown falls out.",
         fix="A system in two variables has answers that are POINTS. Make the last line of every "
             "solution an ordered pair, and ask how many points there should be.",
         probe="Solve y = x^2 and y = x + 2. How many points?"),
